...
// If don't have any error do submit form 
    const submitForm = (e)=>{
        ...
        }else{ //Add visibility class inputs after removing visibility class with hover
   ...
   const inputHover = (e)=>{
    ...
    const inputLeave = (e)=>{
        if(errors[e.target.name]){
         ...
         if(send){
            if(e.target.value == "Your " +e.target.name+'*' || e.target.value == ''){
                setErrors(validationErrors)
            ...
            <div>
                ...
                <form  action="" >
                   ...
                   <div className="contact-form-div">
                        {errors.name&&<div className="errors formActive"><span>{errors.name}</span></div>}
                        <input type="text" style={{backgroundColor: darkBackground,WebkitBoxShadow:darkBackground,}} value={name} onMouseOver={inputHover} onMouseLeave={inputLeave} onKeyUp={validateForm}  onChange={(e)=>setName(e.target.value)}  onFocus={clearInput} onBlur={typeInput} name="name"/>
                   </div>
                    <div className="contact-form-div">
                        {errors.phone &&<label className="errors formActive"><span>{errors.phone}</span></label>}
                        <input type="text" style={{backgroundColor: darkBackground,WebkitBoxShadow:darkBackground}} value={phone} onMouseOver={inputHover} onMouseLeave={inputLeave} onKeyUp={validateForm} onChange={(e)=>setPhone(e.target.value)} onFocus={clearInput} onBlur={typeInput}  name="phone"/>
                   </div>
                   <div className="contact-form-div">
                        {errors.email &&<label className="errors formActive"><span>{errors.email}</span></label>}
                        <input type="email" style={{backgroundColor: darkBackground,WebkitBoxShadow:darkBackground}} value={email} onMouseOver={inputHover} onMouseLeave={inputLeave} onKeyUp={validateForm} onChange={(e)=>setEmail(e.target.value)} onFocus={clearInput} onBlur={typeInput}  name="email"/>
                   </div>
                   <div className="contact-form-div">
                        {errors.message &&<label className="errors formActive"><span>{errors.message}</span></label>}
                        <textarea style={{backgroundColor: darkBackground,WebkitBoxShadow:darkBackground}} value={message} onMouseOver={inputHover} onMouseLeave={inputLeave} onKeyUp={validateForm} onChange={(e)=>setMessage(e.target.value)} onFocus={clearInput} onBlur={typeInput}  name="message" cols="38" rows="5"></textarea>
                   </div>
